...
class A1WalkingVel(A1Walking):
    ...
    def __init__(self, num_envs, horizon, headless, domain_randomization=True, camera_position=(105, 0, 4), camera_target=(95, 0, 0)):
        ...
        IsaacSim.__init__(self, usd_path, self._action_spec, observation_spec, backend, device, collision_between_envs, num_envs, 
                         ...
                         action_type=ActionType.VELOCITY, headless=headless, n_intermediate_steps=1, n_substeps=4, timestep=0.005, 
    ...
    def _set_stiffness_damping(self):
        ...
        self._write_data("joint_damping", torch.full((self.number, self.NUM_JOINTS), 30., device=self._device), env_ids, reapply_after_reset=True)
        self._write_data("joint_stiffness", torch.full((self.number, self.NUM_JOINTS), 0., device=self._device), env_ids, reapply_after_reset=True)
