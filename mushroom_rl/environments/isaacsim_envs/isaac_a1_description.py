...
class IsaacA1Description(IsaacSim):
    def __init__(self, num_envs, horizon, headless):
        ...
        usd_path = "/home/bjarne/GitWorkspace/BachelorThesis/mushroom-rl/isaac_assets/a1_effort/a1/a1.usd"
    ...
    def reward(self, obs, action, next_obs, absorbing):
        pos = self._get_from_obs(next_obs, "body_pos")
        rot = self._get_from_obs(next_obs, "body_rot")
        #
        self._update_air_time()

        reward_pos = 10 * pos[:, 0]
        reward_rot = torch.sum(torch.square(rot), dim=1)
        reward_air_time = torch.clamp(torch.sum(self.foot_air_time - 0.5, dim=1), min=-2., max=8.)
        reward_height = pos[:, 2] # torch.clamp(, min=0., max=0.5) 
        penalty_collision = - self._get_collision_count("lower_body", "groundplane")
        return 10 + reward_pos - reward_rot + 8. * reward_height + penalty_collision + 0.2 * reward_air_time 

    """
    def reward(self, obs, action, next_obs, absorbing):
        pos = self._get_from_obs(next_obs, "body_pos")
        return pos[:, 2]
        #self._get_collision_count("lower_body", "groundplane")
    """
    ...
    def reward(self, obs, action, next_obs, absorbing):
        ...
        #reward_lin_vel_tracking=torch.square(torch.linalg.norm(base_lin_vel_xy, dim=1))
        reward_lin_vel_tracking = self._get_from_obs(next_obs, "body_pos")[:, 0] * 10.
