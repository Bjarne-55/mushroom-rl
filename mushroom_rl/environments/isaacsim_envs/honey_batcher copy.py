...
class HoneyBatcher(IsaacA1Description):
    ...
    def push_robot(self, env_indices):
        max_vel= 1.
        vels = torch_rand_float(-max_vel, max_vel, (env_indices.shape[0], 3), device=self._device)
        extended_vels = self._read_data("body_vel", env_indices)

        if self.np_rng.uniform() < 0.5:
            extended_vels[:, :3] = vels
        else:
            extended_vels[:, :3] += vels * 1.5
