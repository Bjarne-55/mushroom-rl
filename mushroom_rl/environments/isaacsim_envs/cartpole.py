...
class CartPole(IsaacSim):
    ...
    def setup(self, env_indices, obs):
        ...
        cart_joint_pos = 0.25 * (2.0 * torch.rand(num_environments, 1, device=self._device) - 1)
        pole_joint_pos = 0.05 * np.pi * (2.0 * torch.rand(num_environments, 1, device=self._device) - 1)

        cart_joint_vel = 0.25 * (2.0 * torch.rand(num_environments, 1, device=self._device) - 1)
        pole_joint_vel = 0.05 * np.pi * (2.0 * torch.rand(num_environments, 1, device=self._device) - 1)
